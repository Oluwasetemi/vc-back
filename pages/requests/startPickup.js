		...
		<Wrapper>
			<DashboardLayout>
				...
				userId={ singleRequest && singleRequest.user._id}
					top={
						<>
							...
							<div className="buttons flex wrap">
								...
								<p className="reschedule pink" onClick={() => alert('coming soon')}>Reschedule</p>
						...
						<>
							...
							<div className="rhs">
								<div className="list grid first">
									...
									<p className="text bold">{ singleRequest && singleRequest.numberOfItems}</p>
								</div>
								<div className="list grid">
									<p className="text">Type</p>
									<p className="text bold">{ singleRequest && singleRequest.type}</p>
								</div>
								<div className="list grid">
									<p className="text">Location</p>
									<p className="text bold">{ singleRequest && singleRequest.pickupLocation.location}</p>
								</div>
							</div>
							<div className="rhs">
								<div className="list grid first">
									<p className="text">Delivery Date</p>
									<p className="text bold">{ singleRequest && singleRequest.datetimePicked.substring(0,10)}</p>
								</div>
								<div className="list grid">
									<p className="text">Phone Number</p>
									<p className="text bold">{ singleRequest && singleRequest.contactPhoneNumber}</p>
								</div>
								<div className="list grid">
									<p className="text">Subscription</p>
									<p className="text bold">{ singleRequest && singleRequest.user.currentSubscriptionPlan.name}</p>
								</div>
							</div>
						</>
					}
					...
					weight="value"
					text={`User has requested to checkout ${singleRequest && singleRequest.numberOfItems} items from their closet`}
				/>
			</DashboardLayout>
		</Wrapper>
	);
