			...
			<TabPanel value={value} index={0}>
				<div className="flexy">
					{user.closet === null ? (
						...
						user.closet.items.map(item => (
							<SingleItem item={item} userId={user._id} />
						))
					)}
				</div>
			</TabPanel>
				...
				<TabPanel value={value} index={2}>
					<>
						{user.outfit && user.outfit.length === 0 ? (
							...
							user.outfit.map(outfit => (
								<SingleOutfit outfit={outfit} userId={user._id} />
							))
						)}
					</>
				</TabPanel>
